...
"Lua display callback (on_display) failed: %s.  Removing the display callback.\n",
...
void mouse(int button, int state, int xi, int yi)
{
    ...
    if(lua_isfunction(lua_state, -1)) {
        ...
        if(lua_pcall(lua_state,4,0,0)!=0) { 
            char msg[3*80];
...
void passive_motion(int xi, int yi)
{
    ...
    if(lua_isfunction(lua_state, -1)) {
        ...
        if(lua_pcall(lua_state,2,0,0)!=0) { 
            char msg[3*80];
            snprintf(msg,sizeof(msg), 
                    "Lua passive motion callback failed: %s\n",
                    lua_tostring(lua_state,-1));
            b3d_warn(msg);
        }
    }
...
void keyboard(unsigned char key, int xi, int yi)
{
    ...
    if(lua_isfunction(lua_state, -1)) {
        ...
        if(lua_pcall(lua_state,3,0,0)!=0) { 
            char msg[3*80];
            snprintf(msg,sizeof(msg), 
                    "Lua keyboard callback failed: %s\n",
                    lua_tostring(lua_state,-1));
            b3d_warn(msg);
        }
    }
...
void keyboard_up(unsigned char key,int xi,int yi)
{
    ...
    if(lua_isfunction(lua_state, -1)) {
        ...
        if(lua_pcall(lua_state,3,0,0)!=0) { 
            char msg[3*80];
            snprintf(msg,sizeof(msg), 
                    "Lua keyboard-up callback failed: %s\n",
                    lua_tostring(lua_state,-1));
            b3d_warn(msg);
        }
    }
...
void menu_callback(int option)
{
    ...
    if(lua_isfunction(lua_state, -1)) {
        if(lua_pcall(lua_state, /*nargs*/0, /*nresults*/0, /*err_func*/0)!=0) { 
            char msg[3*80];
            snprintf(msg,sizeof(msg), 
                    "Lua menu callback failed: %s\n",
                    lua_tostring(lua_state,-1));
            b3d_warn(msg);
        }
        glutPostRedisplay();
        ...
        if(!lua_isnil(lua_state, -1)) {
            b3d_warn("Warning: menu callback is neither a function nor nil.\n");
        }
        lua_pop(lua_state,1);
    }
}
