...
static int get_line(char buf[], const int bufsize, FILE *file)
{
	line_num++;

	return (fgets(buf, bufsize, file) != NULL) && (!((buf[0] == '#') || is_blank_line(buf)) || get_line(buf, bufsize, file));
}
...
static void calc_face_normal(const mesh_t *mesh, GLfloat n[3], const int fi)
{
	const int *t = mesh->tris[fi].iv;
	int i;
...
static void mesh_drawlist(mesh_t *mesh)
{
	...
	for (i = 0; i < mesh->nt; i++) {
		...
		for (j = 0; j < 3; j++) {
			...
			const vertex_t *v = &mesh->verts[t[j]];
	...
	glInterleavedArrays(GL_N3F_V3F, 0, gl_tris);
...
{
	const mesh_t *m = luaL_checkmesh(L, 1);
	int i;
...
static int mesh_load(mesh_t *mesh, FILE *file)
{
	...
	if (mesh->verts == NULL || mesh->tris == NULL) {
...
static void mesh_draw(const mesh_t *m)
{
