...
static void setprogdir(lua_State *L){
	...
	if((n==0)||(n==nsize)||(!(lb = strrchr(buff, '\\'))))
...
static int gctm(lua_State *L){
	int n;
	for(n = luaL_len(L, 1); n>=1; n--){			/* for each handle, in reverse order */
...
static void setpath(lua_State *L, const char *fieldname, const char *envname1, const char *envname2, const char *def){
	...
	if((!path)||noenv(L))					/* no environment variable? */
