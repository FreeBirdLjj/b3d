...
#define noLuaClosure(f)		((!f)||((f)->c.tt==LUA_TCCL))
...
static int auxgetinfo(lua_State *L, const char *what, lua_Debug *ar, Closure *f, CallInfo *ci){
	...
	for(status = 1; *what; what++){
		...
		case 'n':
			...
			ar->namewhat = ci&&(!(ci->callstatus&CIST_TAIL))&&isLua(ci->previous)? getfuncname(L, ci->previous, &ar->name) : NULL;
			if(!ar->namewhat){
		...
		case OP_GETTABLE:
			{
				...
				const char *vn = op==OP_GETTABLE? luaF_getlocalname(p, t+1, pc) : upvalname(p, t);	/* name of indexed variable */
				kname(p, pc, k, name);
