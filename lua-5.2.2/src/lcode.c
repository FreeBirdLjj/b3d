		...
		if(GET_OPCODE(*previous)==OP_LOADNIL){
			...
			if(((pfrom<=from)&&(from<=pl+1))||
			   ((from<=pfrom)&&(pfrom<=l+1))){	/* can connect both? */
				from = pfrom<from? pfrom : from;	/* from = min(from, pfrom) */
				l = pl>l? pl : l;		/* l = max(l, pl) */
...
static Instruction *getjumpcontrol(FuncState *fs, int pc){
	...
	return (pc>=1)&&(testTMode(GET_OPCODE(*(pi-1))))? pi-1 : pi;
}
...
static void patchlistaux(FuncState *fs, int list, int vtarget, int reg, int dtarget){
	while(list!=NO_JUMP){
		...
		fixjump(fs, list, patchtestreg(fs, list, reg)? vtarget : dtarget);
		list = next;
	}
}
...
static void codecomp(FuncState *fs, OpCode op, int cond, expdesc *e1, expdesc *e2){
	...
	freeexp(fs, e1);
	if((cond==0)&&(op!=OP_EQ)){
		int temp = o1;					/* exchange args to replace by `<' or `<=' */
		o1 = o2;
		o2 = temp;					/* o1 <==> o2 */
		cond = 1;
	}
