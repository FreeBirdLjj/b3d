...
static void save(LexState *ls, int c){
	Mbuffer *b = ls->buff;
	if(luaZ_bufflen(b)>=luaZ_sizebuffer(b)){
		size_t newsize;
		if(luaZ_sizebuffer(b)>=MAX_SIZET>>1)
			lexerror(ls, "lexical element too long", 0);
		newsize = luaZ_sizebuffer(b)<<1;
...
endloop:
	if(seminfo)
		seminfo->ts = luaX_newstring(ls, luaZ_buffer(ls->buff)+(2+sep), luaZ_bufflen(ls->buff)-((2+sep)<<1));
}
		...
		default:
			{
				if(lislalpha(ls->current)){	/* identifier or reserved word? */
					...
					if(isreserved(ts))	/* reserved word? */
						return ts->tsv.extra+FIRST_RESERVED-1;
					return TK_NAME;
				}
				/* single-char tokens (+ - / ...) */
				int c = ls->current;
