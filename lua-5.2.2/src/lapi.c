...
static TValue *index2addr(lua_State *L, int idx){
	...
	if(idx>0){
		...
		return o>=L->top? NONVALIDVALUE : o;
	}
	if(!ispseudo(idx)){	/* negative index */
...
LUA_API int lua_checkstack(lua_State *L, int size){
	...
	if(L->stack_last-L->top>size)	/* stack large enough? */
		...
		res = inuse>LUAI_MAXSTACK-size? 0 : (luaD_rawrunprotected(L, &growstack, &size)==LUA_OK);	/* can grow without overflow? if can, try to grow stack */
	}
...
LUA_API void lua_copy(lua_State *L, int fromidx, int toidx){
	TValue *fr;
	lua_lock(L);
	moveto(L, fr = index2addr(L, fromidx), toidx);
	lua_unlock(L);
}
...
LUA_API lua_CFunction lua_tocfunction(lua_State *L, int idx){
	StkId o = index2addr(L, idx);
	return ttislcf(o)? fvalue(o) : (ttisCclosure(o)? clCvalue(o)->f : NULL);	
}
...
LUA_API lua_State *lua_tothread(lua_State *L, int idx){
	StkId o = index2addr(L, idx);
	return ttisthread(o)? thvalue(o) : NULL;
}
...
LUA_API void lua_pushnumber(lua_State *L, lua_Number n){
	lua_lock(L);
	setnvalue(L->top, n);
	luai_checknum(L, L->top, luaG_runerror(L, "C API - attempt to push a signaling NaN"));
	api_incr_top(L);
	lua_unlock(L);
}
...
LUA_API void lua_callk(lua_State *L, int nargs, int nresults, int ctx, lua_CFunction k){
	StkId func;
	lua_lock(L);
	api_check(L, (!k)||(!isLua(L->ci)), "cannot use continuations inside hooks");
	api_checknelems(L, nargs+1);
	api_check(L, L->status==LUA_OK, "cannot do calls on non-normal thread");
	checkresults(L, nargs, nresults);
	func = L->top-nargs-1;
	if(k&&(L->nny==0)){				/* need to prepare continuation? */
...
LUA_API int lua_pcallk(lua_State *L, int nargs, int nresults, int errfunc, int ctx, lua_CFunction k){
	...
	if((!k)||(L->nny>0)){		/* no continuation or no yieldable? */
...
LUA_API int lua_dump(lua_State *L, lua_Writer writer, void *data){
	...
	api_checknelems(L, 1);
	o = L->top-1;
	status = isLfunction(o)? luaU_dump(L, getproto(o), writer, data, 0) : 1;
	lua_unlock(L);
	return status;
}
