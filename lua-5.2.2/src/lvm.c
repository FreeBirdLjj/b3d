	...
	for(loop = 0; loop<MAXTAGLOOP; loop++){
		...
		if(ttistable(t)){				/* `t' is a table? */
			...
			if((!ttisnil(res))||			/* result is not nil? */
			   (!(tm = fasttm(L, h->metatable, TM_INDEX)))){	/* or no TM? */
				setobj2s(L, val, res);
				return;
			}
...
int luaV_lessthan(lua_State *L, const TValue *l, const TValue *r){
	...
	if((res = call_orderTM(L, l, r, TM_LT))<0)
...
int luaV_lessequal(lua_State *L, const TValue *l, const TValue *r){
	...
	if(ttisstring(l)&&ttisstring(r))
		return l_strcmp(rawtsvalue(l), rawtsvalue(r))<=0;
	if((res = call_orderTM(L, l, r, TM_LE))>=0)		/* first try `le' */
		return res;
	if((res = call_orderTM(L, r, l, TM_LT))<0)		/* else try `lt' */
		luaG_ordererror(L, l, r);
	return !res;
}
	...
	do{
		...
		if((!(ttisstring(top-2))||ttisnumber(top-2))||(!tostring(L, top-1))){
			...
			if(tsvalue(top-1)->len==0)		/* second operand is empty? */
				...
				if(ttisstring(top-2)&&(tsvalue(top-2)->len==0))
...
void luaV_arith(lua_State *L, StkId ra, const TValue *rb, const TValue *rc, TMS op){
	TValue tempb, tempc;
	const TValue *b, *c;
	if((b = luaV_tonumber(rb, &tempb))&&(c = luaV_tonumber(rc, &tempc))){
