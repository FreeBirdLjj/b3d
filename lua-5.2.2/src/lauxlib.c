...
LUALIB_API void luaL_traceback(lua_State *L, lua_State *L1, const char *msg, int level){
	...
	int mark = (numlevels>LEVELS1+LEVELS2)*LEVELS1;
	if(msg)
		lua_pushfstring(L, "%s\n", msg);
...
LUALIB_API int luaL_argerror(lua_State *L, int narg, const char *extramsg){
	...
	if(strcmp(ar.namewhat, "method")==0)
		if(narg==1)					/* error is in the self argument itself? */
...
LUALIB_API char *luaL_prepbuffsize(luaL_Buffer *B, size_t sz){
	...
	if(B->size-B->n<sz){					/* not enough space? */
		char *newbuff;
		size_t newsize = B->size<<1;			/* double buffer size */
		newsize = newsize<B->n+sz? B->n+sz : newsize;	/* not big enough? */
...
static const char *luaL_findtable(lua_State *L, int idx, const char *fname, int szhint){
	...
	do{
		...
		if(lua_isnil(L, -1)){			/* no such field? */
			...
			if(!lua_istable(L, -1)){	/* field has a non-table value? */
...
LUALIB_API void luaL_pushmodule(lua_State *L, const char *modname, int sizehint){
	...
	if(!lua_istable(L, -1)){				/* not found? */
		...
		lua_pushglobaltable(L);
		if(luaL_findtable(L, 0, modname, sizehint))
...
LUALIB_API void luaL_setfuncs(lua_State *L, const luaL_Reg *l, int nup){
	luaL_checkversion(L);
	for(luaL_checkstack(L, nup, "too many upvalues"); l->name; l++){	/* fill the table with given functions */
		int i;
...
LUALIB_API void luaL_checkversion_(lua_State *L, lua_Number ver){
	...
	if((lua_tointeger(L, -1)!=-0x1234)||(lua_tounsigned(L, -1)!=(lua_Unsigned)-0x1234))
