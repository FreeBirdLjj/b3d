...
static int doargs(int argc,  char* argv[]){
	...
	for(i = 1; i<argc; i++){
		...
		if(IS("-"))					/* end of options; use stdin */
			break;
		if(IS("-l"))					/* list */
			++listing;
		else
			if(IS("-o")){				/* output file */
				output = argv[++i];
				if((!output)||(*output==0)||((*output=='-')&&(output[1]!=0)))
				...
				if (IS("-p"))			/* parse only */
					...
					if(IS("-s"))		/* strip debug information */
						stripping = 1;
					else
						if (IS("-v"))	/* show version */
							++version;
						else		/* unknown option */
							usage(argv[i]);
	}
...
static int pmain(lua_State* L){
	...
	if(dumping){
		FILE *D = !output? stdout : fopen(output, "wb");
		if(!D)
			cannot("open");
		lua_lock(L);
