...
static int io_tmpfile(lua_State *L){
	LStream *p = newfile(L);
	p->f = tmpfile();
	return !p->f? luaL_fileresult(L, 0, NULL) : 1;
}
...
static void aux_lines(lua_State *L, int toclose){
	...
	for(i = 1; i<=n; i++)
		lua_pushvalue(L, i+1);				/* copy arguments */
	lua_pushcclosure(L, io_readline, n+3);
}
...
static int read_line(lua_State *L, FILE *f, int chop){
	...
	while(1){
		...
		if(!fgets(p, LUAL_BUFFERSIZE, f)){		/* eof? */
...
static void read_all(lua_State *L, FILE *f){
	...
	while(1){
		...
		if(nr<rlen)
			break;					/* eof? */
		if(rlen<=MAX_SIZE_T>>2)				/* avoid buffers too large */
			rlen <<= 1;				/* double buffer size at each iteration */
	}
	luaL_pushresult(&b);					/* close buffer */
}
